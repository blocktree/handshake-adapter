...
func (wm *WalletManager) GetAddressesByAccount(walletID string) ([]string, error) {
	...
	result, err := wm.NodeClient.Call("getaddressesbyaccount", request)
	if err != nil {
		return nil, err
	}
...
func (wm *WalletManager) ImportPrivKey(wif, walletID string) error {
	...
	_, err := wm.NodeClient.Call("importprivkey", request)
...
func (wm *WalletManager) ImportAddress(address, account string) error {
	...
	_, err := wm.NodeClient.Call("importaddress", request)
...
func (wm *WalletManager) ImportMulti(addresses []*openwallet.Address, keys []string, watchOnly bool) ([]int, error) {
	...
	*/
	...
	result, err := wm.NodeClient.Call("importmulti", request)
	if err != nil {
		return nil, err
	}
...
func (wm *WalletManager) GetCoreWalletinfo() error {
	...
	_, err := wm.NodeClient.Call("getwalletinfo", nil)
...
func (wm *WalletManager) UnlockWallet(passphrase string, seconds int) error {
	...
	_, err := wm.NodeClient.Call("walletpassphrase", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) LockWallet() error {
	...
	_, err := wm.NodeClient.Call("walletlock", nil)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) GetNetworkInfo() error {
	...
	_, err := wm.NodeClient.Call("getnetworkinfo", nil)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) KeyPoolRefill(keyPoolSize uint64) error {
	...
	_, err := wm.NodeClient.Call("keypoolrefill", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) CreateReceiverAddress(account string) (string, error) {
	...
	result, err := wm.NodeClient.Call("getnewaddress", request)
	if err != nil {
		return "", err
	}
...
func (wm *WalletManager) EncryptWallet(password string) error {
	...
	_, err := wm.NodeClient.Call("encryptwallet", request)
	if err != nil {
		return err
	}
	return nil
...
func (wm *WalletManager) AddMultiSigAddress(required uint64, addresses []string) (string, string, error) {
	...
	result, err := wm.NodeClient.Call("addmultisigaddress", request)
	if err != nil {
		return "", "", err
	}
...
func (wm *WalletManager) CreateNewPrivateKey(accountID string, key *owkeychain.ExtendedKey, derivedPath string, index uint64) (string, *openwallet.Address, error) {
	...
	wif, err := wm.Decoder.PrivateKeyToWIF(keyBytes, false)
	...
	address, err := wm.Decoder.PublicKeyToAddress(publicKey, false)
...
func (wm *WalletManager) BackupWalletData(dest string) error {
	...
	_, err := wm.NodeClient.Call("backupwallet", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) DumpWallet(filename string) error {
	...
	_, err := wm.NodeClient.Call("dumpwallet", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) ImportWallet(filename string) error {
	...
	_, err := wm.NodeClient.Call("importwallet", request)
	if err != nil {
		return err
	}
...
func (wm *WalletManager) GetBlockChainInfo() (*BlockchainInfo, error) {
	...
	result, err := wm.NodeClient.Call("getblockchaininfo", nil)
	if err != nil {
		return nil, err
	}
...
func (wm *WalletManager) ListUnspent(min uint64, addresses ...string) ([]*Unspent, error) {
	...
	for i := 0; i <= step; i++ {
		...
		pice, err = wm.NodeClient.listUnspend(searchAddrs...)
		if err != nil {
			return nil, err
		}
...
func (wm *WalletManager) BuildTransaction(utxos []*Unspent, to []string, change string, amount []decimal.Decimal, fees decimal.Decimal) (string, decimal.Decimal, error) {
	...
	rawTx, err := wm.NodeClient.Call("createrawtransaction", request)
	if err != nil {
...
func (wm *WalletManager) SignRawTransactionInCoreWallet(txHex, walletID string, key *hdkeystore.HDKey, utxos []*Unspent) (string, error) {
	...
	result, err := wm.NodeClient.Call("signrawtransaction", request)
	if err != nil {
		return "", err
	}
...
func (wm *WalletManager) SendRawTransaction(txHex string) (string, error) {
	...
	return wm.NodeClient.sendTransaction(txHex)
}
...
func (wm *WalletManager) EstimateFee(inputs, outputs int64, feeRate decimal.Decimal) (decimal.Decimal, error) {
	...
	witness := 101 * int(inputs)
	weight := base*(4-1) + base + witness

	vsize := (weight + 4 - 1) / 4

	trx_fee := decimal.New(int64(vsize), 0)
	trx_fee = trx_fee.Mul(wm.Config.MinFeeRate)
...
func (wm *WalletManager) EstimateFeeRate() (decimal.Decimal, error) {
	feerate, err := wm.NodeClient.getEstimateFeeRate()
	if err != nil {
		return decimal.Decimal{}, err
	}

	if feerate.LessThan(decimal.Zero) {
		return wm.Config.MinFeeRate, nil
	}
...
func (wm *WalletManager) SendToAddress(address string, amount uint64) (string, error) {
	...
	result, err := wm.NodeClient.Call("sendtoaddress", request)
	if err != nil {
		return "", err
	}
