...
func (decoder *TransactionDecoder) CreateRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	return decoder.CreateHNSRawTransaction(wrapper, rawTx)
}

//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {

	return decoder.SignHNSRawTransaction(wrapper, rawTx)
}

//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {

	return decoder.VerifyHNSRawTransaction(wrapper, rawTx)
}
...
//CreateRawTransaction 创建交易单
func (decoder *TransactionDecoder) CreateHNSRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	err = decoder.createHNSRawTransaction(wrapper, rawTx, usedUTXO, outputAddrs)
	if err != nil {
		return err
	}
...
//SignRawTransaction 签名交易单
func (decoder *TransactionDecoder) SignHNSRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			//签名交易
			/////////交易单哈希签名
			sig, err := handshakeTransaction.SignRawTransactionHash(keySignature.Message, keyBytes)
			if err != nil {
...
//VerifyRawTransaction 验证交易单，验证交易单并返回加入签名后的交易单
func (decoder *TransactionDecoder) VerifyHNSRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	signedTrans, pass := handshakeTransaction.CombineAndVerifyRawTransaction(emptyTrans, transHash)
...
//CreateHNSSummaryRawTransaction 创建HNS汇总交易
func (decoder *TransactionDecoder) CreateHNSSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransactionWithError, error) {
		...
		if i == len(sumAddresses)-1 || len(sumUnspents) >= decoder.wm.Config.MaxTxInputs {
			...
			*/
			...
			if sumAmount.GreaterThan(decimal.Zero) {
				...
				createErr = decoder.createHNSRawTransaction(wrapper, rawTx, sumUnspents, outputAddrs)
...
//createHNSRawTransaction 创建HNS原始交易单
func (decoder *TransactionDecoder) createHNSRawTransaction(
	...
	var (
		err              error
		vins             = make([]handshakeTransaction.Vin, 0)
		vouts            = make([]handshakeTransaction.Vout, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for to, amount := range to {
		...
		out := handshakeTransaction.Vout{to, uint64(amount.IntPart())}
	...
	emptyTrans, transHash, err := handshakeTransaction.CreateEmptyRawTransactionAndHash(vins, vouts, lockTime)
	...
	for i, txHash := range transHash {
		...
		unlockAddr = txHash.GetTxAddress()
...
func (decoder *TransactionDecoder) CreateSummaryRawTransactionWithError(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransactionWithError, error) {
	...
	return decoder.CreateHNSSummaryRawTransaction(wrapper, sumRawTx)
}
